...
def test_validate_items_type():
    with pytest.warns(ItemsTypeWarning) as records:
        errors = validate("items_type", additional_valid_types=["boolean"], allow_invalid={"/properties/allow/items"})
...
def test_validate_merge_properties():
    ...
    assert sorted(str(record.message) for record in records) == [
        t(
            'tests/fixtures/schema/merge_properties.json sets "omitWhenMerged" to false or null '
            "at /properties/omitWhenMergedFalse"
        ),
        t(
            'tests/fixtures/schema/merge_properties.json sets "wholeListMerge" to false or null '
            "at /properties/wholeListMergeFalse"
        ),
        t(
            'tests/fixtures/schema/merge_properties.json sets "wholeListMerge", '
            "though the field is not an array of objects, at /properties/array"
        ),
        t(
            'tests/fixtures/schema/merge_properties.json sets both "omitWhenMerged" and "wholeListMerge" '
            "at /properties/both"
        ),
    ]
    assert errors == len(records) == 4
...
def test_validate_object_id():
    ...
    with pytest.warns(ObjectIdWarning) as records:
        errors = validate_object_id(
            path(filepath),
            jsonref.replace_refs(parse(filepath)),
    ...
    assert sorted(str(record.message) for record in records) == [
        t(
            'tests/fixtures/schema/object_id.json is missing "id" in "items/properties" '
            "at /definitions/Missing (from /refMissing)"
        ),
        t('tests/fixtures/schema/object_id.json is missing "id" in "items/properties" at /properties/missing'),
        t(
            'tests/fixtures/schema/object_id.json is missing "id" in "items/required" '
            "at /definitions/Optional (from /refOptional)"
        ),
...
def test_validate_ref_fail():
    ...
    assert sorted(str(record.message) for record in records) == [
        t(
            "tests/fixtures/schema/ref.json has Error while resolving `#/definitions/Fail`: "
            "Unresolvable JSON pointer: '/definitions/Fail' at properties/fail"
        ),
    ]
    assert errors == len(records) == 1
